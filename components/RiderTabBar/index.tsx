...
export default function RiderTabBar({ insets, state, navigation }: BottomTabBarProps) {
  ...
  >
   {state.routes.map((route, k) => {
    ...
    const icon = () => {
     ...
     else if (k === 1) return <MapIcon color={active ? 'primary' : 'caption'} strokeWidth={1.5} />;
     else if (k === 2) return <BagIcon color={active ? 'primary' : 'caption'} strokeWidth={1.5} />;
    ...
    const label = () => {
     if (k === 0)
      ...
      return (
       <Text color={active ? 'primary' : 'caption'} variant={'tabBarLabel'}>
        {t('RiderTabBar.orders')}
       </Text>
      );

     return (
      <Text color={active ? 'primary' : 'caption'} variant={'tabBarLabel'}>
       {t('RiderTabBar.profile')}
      </Text>
     );
    };
